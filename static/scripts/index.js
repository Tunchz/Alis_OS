let container = d3.select("#chart"),
    ...
    alisVisual = createAlisVisual().width(size).height(size).clickFunction(callOnClick);
container.call(alisVisual);
const parse_date = d3.utcParse("%Y-%m-%d %H:%M:%S.%L UTC"),
    parse_date_no_ms = d3.utcParse("%Y-%m-%d %H:%M:%S UTC");
let nodes, filtered_nodes, links, time_frame, time_frame_format, font = new FontFaceObserver("Nunito", {
        weight: 400
    }),
    promises = [];
...
function initChart(e) {
    links = generateLinks(nodes = e), nodes.forEach(e => {
        e.time_created = parse_date(e.created), null === e.time_created && (e.time_created = parse_date_no_ms(e.created)), e.time_updated = parse_date(e.updated), null === e.time_updated && (e.time_updated = parse_date_no_ms(e.updated))
    }), time_frame = getTimeFrame(nodes), time_frame_format = [timeFormat(time_frame[0]), timeFormat(time_frame[1])], alisVisual.createChart(nodes, links, time_frame_format)
}

function updateChart(e) {
    links = generateLinks(nodes = e), nodes.forEach(e => {
        e.time_created = parse_date(e.created), null === e.time_created && (e.time_created = parse_date_no_ms(e.created)), e.time_updated = parse_date(e.updated), null === e.time_updated && (e.time_updated = parse_date_no_ms(e.updated))
    }), time_frame = getTimeFrame(nodes), time_frame_format = [timeFormat(time_frame[0]), timeFormat(time_frame[1])], alisVisual.updateChart(nodes, links, time_frame_format, !0, !0)
}
